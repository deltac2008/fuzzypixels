...
class PiPong:
	...
	def __init__(self):
		...
		# create score text etc
		self.fontstyle = pygame.font.Font(None,17)
...
class Menu():
	...
	def __init__(self, displaySize):
		...
		# should we be drawing the menu?
		self.active = True
		# create the font we'll use
		self.font = font.Font(None, 30)

		# create the text surface
		self.text = self.font.render("Press 1 for a CPU Opponent or 2 for two human players", True, (255,0,0))
		# get the rectangle and center it#
	...
	def handleEvents(self):		
		# handle event, starting with the quit event 
		for event in pygame.event.get():
			if event.type == QUIT:
				pygame.quit()
				sys.exit()
...
class ScoreBoard():
	...
	def incrementScore(self, side):
		if side == "sideA":
			self.sideAValue = ++point
		elif side == "sideB":
			self.sideBValue = ++point
			
	def renderScore(self):
		# display the two players' scores
		playerScore = self.player1Bat.score;
		text = font.render(playerScore,True,(255,255,255),(159,182,205))
		
		# create a rectangle
		textRect = text.get_rect()
		
		# centre the rectangle
		quarterScreenX = self.displaySize[0] / 4
		textRect.centerx = quarterScreenX
		textRect.centery = quarterScreenX / 4 # 16th of height of screen
